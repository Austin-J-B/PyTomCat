...
def _sanitize_for_ndjson(s: str) -> str:
    """Normalize line breaks, avoid exotic separators, strip NULs so each record stays on one line."""
...
async def _log_emails_batch(svc, messages: List[Dict[str, Any]], delay_sec: float = 10.0) -> int:
    ...
    for m in messages:
        mid = str(m.get("id") or "")
        if not mid or mid in seen_input:
            continue
        uniq.append(m)
        seen_input.add(mid)
    ...
    for m in todo:
        try:
            ...
            full = await asyncio.to_thread(
                lambda: svc.users().messages().get(userId="me", id=mid, format="full").execute()
            )
            ...
            ts_received = (
                datetime.utcfromtimestamp(internal_date_ms / 1000).isoformat() + "Z"
            ...
            row = {
                ...
                "id": mid,
                "subject": _sanitize_for_ndjson(subject),
                "from": _sanitize_for_ndjson(from_hdr),
                "ts_received": ts_received,
                "ts_logged": _now_iso(),
                "content": _sanitize_for_ndjson(content),
            }
            ...
            if not locked:
                ...
                try:
                    ...
                    try:
                        ...
                        ch_id = (
                            getattr(_settings, "ch_sandbox", None)
                            or getattr(_settings, "ch_logging", None)
                        )
                        if ch_id:
                            ch = bot.get_channel(int(ch_id))
                    except Exception:
    ...
    try:
        ...
        try:
            if msg:
                ...
                log_action("gmail_log_manual_react", f"msg={getattr(msg,'id',None)}", "+1")
        except Exception:
            pass

        async with _EMAIL_LOG_LOCK:
            log_action("gmail_log_manual_lock", f"req={n}", "acquired")
            svc = await _build_gmail_service(ch)
            ...
            msgs = res.get("messages", []) if isinstance(res, dict) else []
            log_action("gmail_log_manual_list", f"req={n}", f"found={len(msgs)}")

            if not msgs:
                await safe_send(ch, "Logged 0 email(s).")
                try:
                    if msg:
                        await msg.add_reaction("✅")
                        bot = ctx.get("bot")
            ...
            from ..config import settings as _settings
            delay = float(getattr(_settings, 'gmail_log_manual_delay_sec', 0.25) or 0.25)
            logged = await _log_emails_batch(svc, list(msgs)[::-1], delay_sec=delay)
...
async def handle_export_dues_portal(intent, ctx) -> None:
    ...
    if not bot:
        if ch:
            await safe_send(ch, 'Bot context missing.')
        return

    ch_id = getattr(settings, 'ch_due_portal', None)
    if not ch_id:
        await safe_send(ch, 'CH_DUE_PORTAL is not configured.')
        return

    channel = bot.get_channel(int(ch_id))
    if not channel or not hasattr(channel, 'history'):
        await safe_send(ch, f'Could not access channel {ch_id}.')
        return
    ...
    try:
        async for m in channel.history(limit=None, oldest_first=True):
            ...
            rows.append({
                ...
                'created_at': (getattr(m, 'created_at', None) or datetime.now(timezone.utc)).isoformat(),
                'author_id': int(getattr(getattr(m, 'author', None), 'id', 0) or 0),
                'author_username': getattr(getattr(m, 'author', None), 'name', ''),
                'author_display': getattr(getattr(m, 'author', None), 'display_name', '') or getattr(getattr(m, 'author', None), 'name', ''),
        ...
        await safe_send(ch, f'Error reading channel: {e}')
    ...
    try:
        ...
        path = os.path.join(DUES_DIR, f'portal_export-{ts}.ndjson')
        with open(path, 'w', encoding='utf-8') as f:
            for r in rows:
                f.write(json.dumps(r, ensure_ascii=False) + "\n")
        await safe_send(ch, f'Exported {len(rows)} messages to {path}.')
        log_action('dues_export', f'ch={ch_id}', f'rows={len(rows)} path={path}')
    except Exception as e:
        await safe_send(ch, f'Export error: {e}')
        log_action('dues_export_error', f'ch={ch_id}', str(e))
